...
var KTAppLayoutBuilder = function() {
	...
	var handleEngageClose = function() {	
		engageClose.addEventListener("click", function (e) {
			e.preventDefault();

			const date = new Date(Date.now() + 1 * 24 * 60 * 60 * 1000); // 1 days from now
			KTCookie.set("app_engage_closed", "1", {expires: date});
			engage.remove();
		});
	}
	...
	return {
		...
		init: function() {
			...
			engageClose = document.querySelector('#kt_app_engage_close');
